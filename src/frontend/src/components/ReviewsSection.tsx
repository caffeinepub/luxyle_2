...
const SEED_REVIEWS = [
  {
    ...
    rating: 5,
    review:
      "Luxyle completely transformed my living space. The décor is elegant and premium.",
  },
  {
    name: "Rahul M.",
    rating: 5,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {allReviews.map((review, i) => (
            <ReviewCard key={`review-${i}-${review.name}`} {...review} />
          ))}
          {isLoading && (
            <div
              key="skeleton-loading"
              className="bg-ivory p-8 shadow-luxury border-t-2 border-beige-dark"
            >
